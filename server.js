...
if(process.env.DISABLE_DAILY_CRON!=='true')cron.schedule('0 22 * * *',()=>deliver(ist()).then(x=>console.log('10PM daily delivery',x)).catch(console.error),{timezone:'Asia/Kolkata'});
app.listen(PORT,()=>console.log(`SMIT master server on ${PORT}; 10PM cron ${process.env.DISABLE_DAILY_CRON==='true'?'OFF':'ON'}`));
