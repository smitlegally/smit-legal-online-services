...
function transporter(){if(!process.env.SMTP_HOST||!process.env.SMTP_USER||!process.env.SMTP_PASS)return null;return nodemailer.createTransport({host:process.env.SMTP_HOST,port:Number(process.env.SMTP_PORT||587),secure:String(process.env.SMTP_SECURE||'false')==='true',auth:{user:process.env.SMTP_USER,pass:process.env.SMTP_PASS},connectionTimeout:15000,greetingTimeout:15000,socketTimeout:20000})}
async function sendViaResend({r,file,date}){const key=process.env.RESEND_API_KEY;if(!key)return null;const to=process.env.REPORT_EMAIL||'smitlegally@gmail.com';const from=process.env.RESEND_FROM||'SMIT Legal <onboarding@resend.dev>';const content=fs.readFileSync(file).toString('base64');const body={from,to,subject:`SMIT Daily Sales — ${date}`,text:`Paid: ${r.paid.length}\nGovernment Charges: ₹${r.gov}\nSmit Income: ₹${r.income}\nTotal Received: ₹${r.total}`,attachments:[{filename:path.basename(file),content}]};const resp=await axios.post('https://api.resend.com/emails',body,{headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},timeout:20000});if(!resp.data?.id)throw new Error('Resend did not return an email id');return resp.data.id}
async function deliver(date){const {r,file}=await makeXlsx(date);const out={date,file,email:'not configured',whatsapp:'not configured'};if(process.env.RESEND_API_KEY){try{out.email='sent';out.emailId=await sendViaResend({r,file,date})}catch(e){out.email='failed: '+(e.response?.data?.message||e.message)}}else{const t=transporter();if(t){try{await t.sendMail({from:process.env.SMTP_FROM||process.env.SMTP_USER,to:process.env.REPORT_EMAIL||'smitlegally@gmail.com',subject:`SMIT Daily Sales — ${date}`,text:`Paid: ${r.paid.length}\nGovernment Charges: ₹${r.gov}\nSmit Income: ₹${r.income}\nTotal Received: ₹${r.total}`,attachments:[{filename:path.basename(file),path:file}]});out.email='sent'}catch(e){out.email='failed: '+e.message}}}
